...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  void loginByThirdParty({String fgName, String fgEmail, String thirdPartyID, String profilePic, String loginType}) async {
    setState(() {
      _loading = true;
    });
    try {
  ...
  Future<void> _login() async {
    ...
    if (result.status == LoginStatus.success) {
      ...
      if (fgName != null && fgEmail != null && thirdPartyID != null && profilePic != null) {
        loginByThirdParty(fgName: fgName, fgEmail: fgEmail, thirdPartyID: thirdPartyID, profilePic: profilePic, loginType: '1');
      }
    } else {}
  }
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                        ...
                        children: [
                          ...
                          Align(
                            ...
                            child: Row(
                              children: [
                                ...
                                SizedBox(width: 4),
                                AppText(text: 'I agree the', color: Colors.grey.shade500, fontSize: 11),
                                AppText(text: 'Terms & Conditions', color: Colors.grey.shade800, fontSize: 11),
                              ],
                            ),
                          ),
                          SizedBox(height: 20),
                          CustomSubmitButton(
                            ...
                            onPressed: () {
                              ...
                              if (_validate(name: name.text, email: email.text, phoneNumber: phoneNumber.text, password: password.text, isCheckTerm: checkValue)) {
                                createUser();
                              }
                            },
                          ),
                ...
                GestureDetector(
                  child: AppText(text: 'Already have an account? Login', color: kPrimaryColor2, fontSize: 12),
                  onTap: () {
                    FocusScope.of(context).requestFocus(FocusNode());
  ...
  bool _validate({String name, String email, String phoneNumber, String password, bool isCheckTerm}) {
    if (name.isEmpty && email.isEmpty && password.isEmpty && phoneNumber.isEmpty) {
      ...
      Toasty.showtoast('Password Must Contains 8 Letters');
      return false;
    } else if (isCheckTerm == false) {
      Toasty.showtoast('Please agree The Terms & Conditions');
      return false;
    } else {
      return true;
    }
